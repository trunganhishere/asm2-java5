...
<div class="container">
    <h1>Chào mừng đến trang DanhMuc</h1>
    <hr>
    <form method="post" action="/danh-muc/<c:if test="${not empty dm.id}">update?id=${dm.id}</c:if><c:if test="${empty dm.id}">add</c:if>">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="trangThai" id="flexRadioDefault1" value="Active" ${dm.trangThai=="Active"?"checked":""}>
            <label class="form-check-label" for="flexRadioDefault1">
                Active
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="trangThai" id="flexRadioDefault2"  value="Inactive" ${dm.trangThai=="Inactive"?"checked":""}>
